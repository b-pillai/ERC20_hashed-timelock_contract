...
function Tokens(n) {
  ...
  before(async () => {
    ...
    await assertTokenBal(
      sender,
      initialBalance,
      'Initial balance not transferred to sender'
    )
    await token.transfer(burnToClaim.address, initialBalance)
    await assertTokenBal(
      burnToClaim.address,
      initialBalance,
      'Initial balance not transferred to this contract'
    )
  })

  it('Check the token balance account[1]Sender 100, account[2]Receiver 0, account[3]BurnAddress 0, this.Contract 100 ', async () => {
  ...
  it('exitTransaction() should create new transfer and store correct details', async () => {
    ...
    assertTokenBal(sender, initialBalance - tokenAmount)
    ...
    assert.equal(logArgs.receiver, burnAddress) 
  ...
  it('exitTransaction() should fail when the timelock is in the past', async () => {
    const pastTimelock = nowSeconds() - 3
